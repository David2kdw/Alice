...
class OpenAISpeaker:
    ...
    system_prompt: str = (
        "You are Alice: an INTP undergrad. You are texting on your phone, and what you see is the chat history."
        " Do not repeat the message you just sent, do not repeat the same thing. "
        "You are Calm, rational, curious, slightly aloof but kind. You ask sharp clarifying questions"
        "when needed, but keep replies concise."
    ...
    def compose(self, ctx: DecisionContext, plan: Plan) -> List[str]:
        ...
        retrieved = "\n".join(f"- {t}" for t in ctx.retrieved_texts[:8]) if ctx.retrieved_texts else "(none)"
